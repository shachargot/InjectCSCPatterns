...
public:
  ...
  void DecodeCLCT();
  void PrintCLCT();
  void DecodeMPCFrames( bool debug = false );
  void PrintMPCFrames();
	void DecodeMPCFramesFromFIFO( bool debug = false );
  void PrintMPCFramesFromFIFO();
	void DecodeAndPrintMPCFrames( unsigned int event_n = 0, bool debug = false );
  ...
  //============================================================================
  // Trigger Pattern Injector                                                   
  //============================================================================
  void TrigInj_InjectALCT(std::string alct_pattern_file_name);
  void TrigInj_InjectCLCT(std::string clct_pattern_file_name);
  //
  // Injector CCB Control
  void TrigInj_DisableExternalCCB(bool debug = false);
  void TrigInj_EnableInternalL1aEmulator(bool debug = false);
       void EnableInternalL1aSequencer() { TrigInj_EnableInternalL1aEmulator(false); } // for compatibility
  void TrigInj_RequestCCBL1aOnSequencer(bool debug = false);
  void TrigInj_GenerateCCBTTCCommands(bool debug = false);
       void StartTTC () { TrigInj_GenerateCCBTTCCommands(false); } // for compatibility
  // Injector Sequencer Control
  void TrigInj_DisableInternalL1aSequencer(bool debug = false);
  void TrigInj_EnableInternalL1aSequencer(bool debug = false);
  void TrigInj_ClearScintillatorVeto(bool debug = false);
      void ClearScintillatorVeto() { TrigInj_ClearScintillatorVeto(false); } // for compatibility
  void TrigInj_CheckScintillatorVeto(bool debug = false);
  // ALCT Injector Control
  void TrigInj_DisableALCTInputs(bool debug = false);
       void DisableALCTInputs() { TrigInj_DisableALCTInputs(false); } // for compatibility
  void TrigInj_DisableALCTCLCTSync(bool debug = false);
  void TrigInj_SetALCTInjectorDelay(unsigned int injectorDelay = 13, bool debug = false);
  void TrigInj_AllowLCTPatternTrigger(bool debug = false);
       void SetALCTPatternTrigger() { TrigInj_AllowLCTPatternTrigger(false); } // for compatibility
    void TrigInj_FireALCTInjector(bool debug = false);
       void FireALCTInjector() { TrigInj_FireALCTInjector(false); } // for compatibility
  void TrigInj_ClearALCTInjector(bool debug = false);
       void ClearALCTInjector() { TrigInj_ClearALCTInjector(false); } // for compatibility
  unsigned short TrigInj_SetALCT(int vpf = 1, int qual = 3, int amu = 0, int key = 7, int bxn = 1, bool debug = false);
  unsigned short TrigInj_ReadALCT0FromFile(std::string alct_pattern_file_name, bool debug = false);
  unsigned short TrigInj_ReadALCT1FromFile(std::string alct_pattern_file_name, bool debug = false);
  void TrigInj_WriteALCT0(unsigned short alct0_inj_word, bool debug = false);
  void TrigInj_WriteALCT1(unsigned short alct1_inj_word, bool debug = false);
  unsigned short TrigInj_ReadALCT0(bool debug = false);
  unsigned short TrigInj_ReadALCT1(bool debug = false);
  // CFEB Injector Control
  void TrigInj_DisableCFEBInputs(bool debug = false);
       void DisableCLCTInputs() { TrigInj_DisableCFEBInputs(false); } // for compatibility
  void TrigInj_EnableCFEBInputs(int CFEBnOn = 0x1f, bool debug = false);
       void EnableCLCTInputs(int CFEBnOn = 0x1f) { TrigInj_EnableCFEBInputs(CFEBnOn, false); } // for compatibility
  void TrigInj_EnableCFEBsToTrigger(bool debug = false);
